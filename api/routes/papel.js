...
module.exports = (pool) => {
  router.get('/', async (req, res) => {
    try {
      const { rows } = await pool.query('SELECT u.email, u.nome_completo, p.nome_papel FROM papel e INNER JOIN papel p ON e.id_papel = p.id_papel INNER JOIN usuario u ON p.email = u.email');
      res.json(rows);
    } catch (error) {
  ...
  router.get('/:email', async (req, res) => {
    ...
    try {
      const { rows } = await pool.query('SELECT u.email, u.nome_completo, p.nome_papel FROM usuario u INNER JOIN papel p ON u.email = p.email INNER JOIN papel e ON p.id_papel = e.id_papel WHERE u.email = $1', [email]);
      res.json(rows);
    } catch (error) {
