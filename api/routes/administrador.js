...
module.exports = (pool) => {
  ...
  router.get('/:email', async (req, res) => {
    ...
    try {
      const { rows } = await pool.query("SELECT usuario.email, usuario.nome_completo,usuario.departamento, papel.nome_papel FROM usuario JOIN papel ON usuario.email = papel.email where papel.nome_papel = 'Administrador' and usuario.email = $1", [email]);      res.json(rows);
    } catch (error) {
